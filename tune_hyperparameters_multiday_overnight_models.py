...
def get_batch(split, skip=0):
    ...
    for i in ix:
        if count < batch_size:
            ...
            gptconf1 = gptconfigs[list(gptconfigs.keys())[0]]
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        with ctx:
            ...
            for mod_ind in range(len(models.keys())):
                # if mod_ind == 1:
                m = mod_list[mod_ind]
                logitss[m], losses[m] = models[m](X2, Y2)
                losses[m] = losses[m] / gradient_accumulation_steps
                # else:
    ...
    if iter_num % log_interval == 0 and master_process:
        ...
        for m in models.keys():
            if not np.isnan(losses[m].item()):
                lossf[m] = losses[m].item() * gradient_accumulation_steps
        ...
        print(f"iter {iter_num}: loss {lossf[list(lossf.keys())[0]]:.4f}, time {dt*1000:.2f}ms, mfu {running_mfu*100:.2f}, loss2 {lossf[list(lossf.keys())[1]]:.4f}, mfu2 {running_mfu2*100:.2f}%")
    iter_num += 1
    local_iter_num += 1
