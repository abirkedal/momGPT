...
def get_batch(split, skip_last=0):
    t = time.time()
    if split == 'train':
    ...
    for i in ix:
        if count < batch_size:
            ...
            if ds0 - gptconf.n_embd > 0:
                ...
                if skip_last > 0:
                    xd2 = np.clip(data[j:(j+gptconf.n_embd), :-1][:,:-skip_last], -input_clip, input_clip)
                    dm = data[j:(j+gptconf.n_embd), 1:]
                    yd2 = np.delete(dm, range(dm.shape[1]-1-skip_last, dm.shape[1]-1), 1).copy()
                    zd2 = yd2.copy()
                    # yd2 = data[j:(j+gptconf.n_embd), 1:][:,:-skip_last].copy()
                    # zd2 = data[j:(j+gptconf.n_embd), 1:][:,:-skip_last].copy()
                else:
                    xd2 = np.clip(data[j:(j+gptconf.n_embd), :-1], -input_clip, input_clip)
                    yd2 = data[j:(j+gptconf.n_embd), 1:].copy()
                    zd2 = data[j:(j+gptconf.n_embd), 1:].copy()
                # print(xd)
                # print(zd)
...
def estimate_loss():
    ...
    for split in ['train', 'val']:
        ...
        for k in range(eval_iters):
            X1, Y1, Z1, X2, Y2, Z2 = get_batch(split, skip_last=skip_last_val)
...
X, Y, Z, X2, Y2, Z2 = get_batch('train', skip_last=skip_last_val) # fetch the very first batch
...
while True:
    ...
    for micro_step in range(gradient_accumulation_steps):
        ...
        X, Y, Z, X2, Y2, Z2 = get_batch('train', skip_last=skip_last_val)
