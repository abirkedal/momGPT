...
def get_whole_split(split):
    ...
    ix = range(len(tickers)) # torch.randint(len(tickers), (2 * batch_size,))

    xl = []
    yl = []

    t1 = time.time()
    print('get_whole_split', split)
    ...
    for i in ix:
        ...
        if ds0 - gptconf.n_embd > 0:
            ...
            for j in range(ds0 - gptconf.n_embd):
                # print('j', j, ds0 - gptconf.n_embd, i, ticker, data[j:(j+gptconf.n_embd), :-1].shape, gptconf.n_embd)
...
# X, Y = get_whole_split('train') # fetch the very first batch
...
print(local_iter_num,  max_iters)
while local_iter_num < max_iters:
    ...
    if master_process:
        losses, target_losses = compute_loss()
